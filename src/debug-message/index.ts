...
export abstract class DebugMessage {
    ...
    abstract line(document: TextDocument, selectionLine: number, selectedVar: string): number
    abstract spacesBefore(document: TextDocument, line: number, tabSize: number): string
    abstract detectAll(document: TextDocument, tabSize: number, delemiterInsideMessage: string, quote: string): Message[]
    abstract enclosingBlockName(document: TextDocument, lineOfSelectedVar: number, blockType: BlockType): string
    closingElementLine(document: TextDocument, lineNum: number, locElement: LocElement): number {
    ...
    locOpenedClosedElementOccurrences(loc: string, locElement: LocElement) {
        let openedElementOccurrences: number = 0
        let closedElementOccurrences: number = 0
        const openedElement: RegExp = locElement === LocElement.Parenthesis ? /\(/g : /{/g
        const closedElement: RegExp = locElement === LocElement.Parenthesis ? /\)/g : /}/g
