...
export class JSDebugMessage extends DebugMessage {
    ...
    msg(
        ...
        const classThatEncloseTheVar: string = this.enclosingBlockName(document, lineOfSelectedVar, "class")
        const funcThatEncloseTheVar: string = this.enclosingBlockName(document, lineOfSelectedVar, "function")
        const lineOfLogMsg: number = this.line(document, lineOfSelectedVar, selectedVar)
        const spacesBeforeMsg: string = this.spacesBefore(document, lineOfSelectedVar, tabSize)
        const fileName = document.fileName.includes("/")
            ? document.fileName.split("/")[document.fileName.split("/").length - 1]
            : document.fileName.split("\\")[document.fileName.split("\\").length - 1]
        if (!includeFileNameAndLineNum && !insertEnclosingFunction && !insertEnclosingClass && logMessagePrefix.length === 0) {
            logMessagePrefix = `${delemiterInsideMessage} `
        }
        const debuggingMsg: string = `ray(${quote}${logMessagePrefix}${
            logMessagePrefix.length !== 0 && logMessagePrefix !== `${delemiterInsideMessage} ` ? ` ${delemiterInsideMessage} ` : ""
        }${includeFileNameAndLineNum ? `file: ${fileName} ${delemiterInsideMessage} line ${lineOfLogMsg + 1} ${delemiterInsideMessage} ` : ""}${
            insertEnclosingClass ? (classThatEncloseTheVar.length > 0 ? `${classThatEncloseTheVar} ${delemiterInsideMessage} ` : ``) : ""
        }${
            insertEnclosingFunction ? (funcThatEncloseTheVar.length > 0 ? `${funcThatEncloseTheVar} ${delemiterInsideMessage} ` : "") : ""
        }${selectedVar}${quote}, $${selectedVar});`
        if (wrapLogMessage) {
            // 16 represents the length of console.log("");
            const wrappingMsg: string = `ray(${quote}${logMessagePrefix} ${"-".repeat(debuggingMsg.length - 16)}$${quote});`
            textEditor.insert(
                new vscode.Position(lineOfLogMsg >= document.lineCount ? document.lineCount : lineOfLogMsg, 0),
    ...
    line(document: TextDocument, selectionLine: number, selectedVar: string): number {
        if (selectionLine === document.lineCount - 1) {
            return selectionLine
        }
        const multilineParenthesisVariableLine = this.getMultiLineVariableLine(document, selectionLine, LocElement.Parenthesis)
        const multilineBracesVariableLine = this.getMultiLineVariableLine(document, selectionLine, LocElement.Braces)
        ...
        if (this.lineCodeProcessing.isObjectLiteralAssignedToVariable(`${currentLineText}\n${nextLineText}`)) {
            ...
            return multilineParenthesisVariableLine !== null && this.lineText(document, multilineParenthesisVariableLine - 1).includes("{")
                ? multilineParenthesisVariableLine
                : selectionLine + 1
        } else if (this.lineCodeProcessing.isFunctionDeclaration(currentLineText)) {
            if (multilineParenthesisVariableLine !== null && this.lineText(document, multilineParenthesisVariableLine - 1).includes("{")) {
        ...
        } else if (multilineParenthesisVariableLine !== null && this.lineText(document, multilineParenthesisVariableLine - 1).includes("{")) {
            return multilineParenthesisVariableLine
    ...
    private objectFunctionCallLine(document: TextDocument, selectionLine: number, selectedVar: string): number {
        let currentLineText: string = document.lineAt(selectionLine).text
        let nextLineText: string = document.lineAt(selectionLine + 1).text.replace(/\s/g, "")
        ...
        const { openedElementOccurrences, closedElementOccurrences } = this.locOpenedClosedElementOccurrences(currentLineText, LocElement.Parenthesis)
        ...
        if (totalOpenedParenthesis !== totalClosedParenthesis || currentLineText.endsWith(".") || nextLineText.trim().startsWith(".")) {
            while (currentLineNum < document.lineCount) {
    ...
    private functionAssignmentLine(document: TextDocument, selectionLine: number): number {
        const currentLineText = document.lineAt(selectionLine).text
        if (/{/.test(currentLineText)) {
            return this.closingElementLine(document, selectionLine, LocElement.Braces) + 1
        } else {
            const closedParenthesisLine = this.closingElementLine(document, selectionLine, LocElement.Parenthesis)
            return this.closingElementLine(document, closedParenthesisLine, LocElement.Braces) + 1
        }
    }
    ...
    private getMultiLineVariableLine(document: TextDocument, lineNum: number, blockType: LocElement): number | null {
    ...
    private functionOpenedBraceLine(docuemt: TextDocument, line: number) {
        ...
        while (line < docuemt.lineCount) {
            const { openedElementOccurrences, closedElementOccurrences } = this.locOpenedClosedElementOccurrences(this.lineText(docuemt, line), LocElement.Braces)
            nbrOfOpenedBraces += openedElementOccurrences
            nbrOfClosedBraces += closedElementOccurrences
            if (nbrOfOpenedBraces - nbrOfClosedBraces === 1 || nbrOfOpenedBraces - nbrOfClosedBraces === 0) {
                return line
            }
            line++
        }
    ...
    spacesBefore(document: TextDocument, line: number, tabSize: number): string {
        ...
        if (
            (!this.lineCodeProcessing.isFunctionAssignedToVariable(currentLine.text) && this.lineCodeProcessing.doesContainsNamedFunctionDeclaration(currentLine.text)) ||
    ...
    enclosingBlockName(document: TextDocument, lineOfSelectedVar: number, blockType: BlockType): string {
            ...
            switch (blockType) {
                case "class":
                    if (this.lineCodeProcessing.doesContainClassDeclaration(currentLineText)) {
                        if (lineOfSelectedVar > currentLineNum && lineOfSelectedVar < this.closingElementLine(document, currentLineNum, LocElement.Braces)) {
    ...
    detectAll(document: TextDocument, tabSize: number, delemiterInsideMessage: string, quote: string): Message[] {
        const documentNbrOfLines: number = document.lineCount
        const logMessages: Message[] = []
        for (let i = 0; i < documentNbrOfLines; i++) {
            const quickRayDumperMessage: RegExp = /ray\(/
            if (quickRayDumperMessage.test(document.lineAt(i).text)) {
                const logMessage: Message = {
                    spaces: "",
                    lines: [],
                }
                logMessage.spaces = this.spacesBefore(document, i, tabSize)
                const closedParenthesisLine = this.closingElementLine(document, i, LocElement.Parenthesis)
                let msg = ""
